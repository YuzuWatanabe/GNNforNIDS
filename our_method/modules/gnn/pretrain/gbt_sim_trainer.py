...
class Trainer:
    def __init__(self, config, gnn_module, llm_module, params, optimizer, scheduler, scaler, device):
    ...
    def train(self, data_loader, epoch):
        ...
        for idx, (batched_raw, batched_graph, _) in enumerate(data_loader):
            ...
            with torch.amp.autocast(self.device.type, enabled=self.config.ENABLE_AMP):
                ...
                loss_llm = F.l1_loss(llm_z, llm_z_aug, reduction='none')

                # lossの加算に指数移動平均を適用
                total_loss = loss_gnn.mean() + loss_llm.mean()
