...
class GNNEncoder(torch.nn.Module):
    ...
    def forward(self, batched_graph, device):
        ...
        h_neg, _ = self.encoder(batch_g_aug, x_aug, self.training)
...
class Trainer:
    def __init__(self, config, gnn_module, mocov3, discliminator, params, optimizer, scheduler, scaler, device):
        ...
        self.contrast_model = DifferentExtendContrast(loss=HDMI()).to(self.device) # 損失関数(Barlow Twin)
    ...
    def train(self, data_loader, epoch):
        ...
        for idx, (batched_raw, batched_graph, _) in enumerate(data_loader):
            ...
            with torch.amp.autocast(self.device.type, enabled=self.config.ENABLE_AMP):
                ...
                # lossの加算に指数移動平均を適用
                total_loss = 0.5*loss_gnn + 0.5*loss_llm
