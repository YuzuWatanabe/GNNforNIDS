...
from modules.gnn.pretrain.losses.losses import Loss
from modules.gnn.pretrain.samplers import get_sampler

# Graph Barlow Twin Lossの対比用関数
...
class DifferentExtendContrast(torch.nn.Module):
    def __init__(self, loss: Loss, **kwargs):
        super(DifferentExtendContrast, self).__init__()
        self.loss = loss
        self.kwargs = kwargs

    def forward(self, pos, neg):
        l = self.loss(anchor=pos, sample=neg, **self.kwargs)
